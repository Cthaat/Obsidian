...
def build_sources() -> list[dict]:
    ...
    chapter_dir = SOURCE_ROOT / "计算机组成原理基础考点讲解" / "第四章 指令系统"
    sources: list[dict] = [
        {
            "phase": "textbook",
            "path": textbook,
            "pages": list(range(159, 206)),
            "force_ocr": True,
        }
    ]
...
def main() -> None:
    ...
    for source_index, source in enumerate(build_sources()):
        ...
        if text_path.exists() and len(existing_contacts) == expected_contacts:
            contacts = [str(path.relative_to(OUT)) for path in existing_contacts]
            entry = {
                "phase": source["phase"],
                "name": path.name,
    ...
    (OUT / "manifest.json").write_text(
        json.dumps({"summary": summary, "sources": manifest}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
