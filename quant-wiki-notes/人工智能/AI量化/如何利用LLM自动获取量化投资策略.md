...
本文介绍了一个全新的量化投资框架，旨在借助**大型语言模型（LLMs）** 与**多代理（multi-agent）** 的协同来挖掘并优化股票

> [!note] 股票
> 公司所有权的凭证，代表股东对公司资产和收益的权益
投资策略中的Alpha因子。该方法从多模态金融数据（财报、价格、新闻、图表）中筛选并生成多元化的Alpha集合，再通过多代理机制根据实时市场情境动态评估与择优，从而构建适应性更强、表现更稳定的投资组合。
...
2. **金融领域LLM**：尽管通用大模型研究火热，但在金融任务（尤其是量化交易

> [!note] 量化交易
> 使用数学模型和算法进行交易决策的方法
）中仍属早期探索，许多研究集中在财务文本摘要或新闻情绪分析；  
