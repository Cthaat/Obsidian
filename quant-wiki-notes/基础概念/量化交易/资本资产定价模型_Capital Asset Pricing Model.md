...
> [!note] 💡 概念解析
> CAPM描述资产预期收益与系统性风险(beta)的关系：E(R) = Rf + β(E(Rm) - Rf)。beta=1与市场同步，>1波动更大。

资本资产定价模型（CAPM）描述了系统性风险（即投资的一般风险）与资产（尤其是股票

> [!note] 股票
> 公司所有权的凭证，代表股东对公司资产和收益的权益
）预期收益之间的关系。这是一个金融模型，建立了投资所需回报与风险之间的线性关系。
