...
> 注释：这些指标都是在帮你判断“是不是偏得太过头”。它们只是不同角度的均值回归信号，不是单独一个指标就能保证赚钱。

日内交易涉及在同一交易日内买卖金融工具，通常只持有几分钟或几小时. 均值回归在日内交易策略中发挥着关键作用，因为它帮助交易者利用短期价格波动获利。

> 注释：这里是在说均值回归特别适合短周期。因为短线里价格更容易出现过度反应，回拉也更常见。
...
> 注释：摆动交易比日内更慢一些，但仍然会利用“偏离后回归”的机会，只是持有时间更长。
...
在外汇交易中，均值回归策略旨在利用货币对回归其历史均值或平均价格的倾向。均值回归在使用技术分析指标识别短期机会时特别有用。

> 注释：外汇市场里，货币对也会围绕某些长期均值波动，所以均值回归不仅仅适用于股票。
...
> 注释：这几条总结的是均值回归为什么受欢迎：容易规则化、适用面广、方便加风控，而且在震荡市里常常更有优势。
...
> 注释：这些限制说明，均值回归不是“看到偏离就冲进去”。它最怕趋势太强、成本太高和消息面突然改变市场结构。

## 什么是均值回归策略？

均值回归策略是一种交易方法，利用金融资产回归其历史均值或平均价格的倾向。该策略旨在识别被显著高估或低估的资产，并根据价格回归均值的预期进行交易。

> 注释：这一句就是策略核心：先判断偏离，再押注回归。它本质上是逆着极端情绪做交易。

## 什么是最适合使用均值回归交易的资产？

选择使用均值回归策略交易的资产依赖于多种因素，例如市场条件、实体的交易和投资专业知识，以及风险韧性。

> 注释：这里是在说不是所有资产都一样适合做均值回归。流动性、波动结构和市场机制都会影响这种策略能不能跑通。
...
> 注释：这是两个相反的交易哲学。趋势跟随是“顺着走”，均值回归是“偏了就等它回”。
...
> 注释：总结一句话，均值回归适合“偏离太大时反着做”，但它高度依赖市场环境和成本控制。没有风控，策略很容易在趋势市里失效。
...
### 为何在量化/投资里重要
价格/价差「拉回中枢」的假设支撑配对、网格、某些短线；趋势市会持续单边打穿。
